...
**⚠️ CAPTCHA handling:** MassCourts shows a reCAPTCHA on the home page. Use the 2Captcha API to solve it programmatically.

- **API Key:** `0e4b6415b991cfcbcc1aabf47f2e7561`
- **Sitekey:** `6Ld9CuYlAAAAAPTI6mtGcd0843Pj2knpu9VsmS7Y`

**VERIFIED WORKING METHOD (April 2026):**

**Critical:** Navigate fresh to `home.page.3`. The reCAPTCHA widget creates a `g-recaptcha-response` textarea natively. ONLY set value on the EXISTING textarea — never create a new one manually. After setting value, click the submit button.

```bash
# Step 1: Get token from 2captcha (tokens arrive in ~5 seconds)
APIKEY="0e4b6415b991cfcbcc1aabf47f2e7561"
SITEKEY="6Ld9CuYlAAAAAPTI6mtGcd0843Pj2knpu9VsmS7Y"
PAGEURL="https://www.masscourts.org/eservices/home.page.3"
REQ=$(curl -s "https://2captcha.com/in.php?key=${APIKEY}&method=userrecaptcha&googlekey=${SITEKEY}&pageurl=${PAGEURL}&json=1")
REQUEST_ID=$(echo $REQ | python3 -c "import sys,json; print(json.load(sys.stdin)['request'])")
for i in $(seq 1 12); do
  sleep 5
  RESULT=$(curl -s "https://2captcha.com/res.php?key=${APIKEY}&action=get&id=${REQUEST_ID}&json=1")
  STATUS=$(echo $RESULT | python3 -c "import sys,json; print(json.load(sys.stdin).get('status',0))")
  if [ "$STATUS" = "1" ]; then
    echo $(python3 -c "import sys,json; print(json.load(sys.stdin)['request'])" <<< "$RESULT") > /tmp/captcha_token.txt
    echo "READY poll $i"; break
  fi
done
```

```bash
cat /tmp/captcha_token.txt  # copy token value
```

```javascript
// Step 2: Inject into EXISTING textarea (pass as string literal — Chrome extension filters output not input)
const token = "FULL_TOKEN_HERE";
const ta = document.getElementById('g-recaptcha-response');
if (ta) { ta.value = token; }
document.querySelector('input[name="linkFrag:beginButton"]').click();
// Wait ~3 seconds, then verify document.title === "Search - Massachusetts Trial Court N6"
```

Never stop and ask the user to intervene.
...
**Step 4 — Open the complaint image FIRST and check assessed value:**

The complaint (first docket entry labeled "Image" next to "Complaint filed") contains the assessed land value and full property description. **This is the first thing to check — if the land value is too low, skip the case immediately without further research.**

Click the complaint image link by its element ID:
```javascript
// Find the first image link in the docket
const imgLink = Array.from(document.querySelectorAll('a')).find(a => a.textContent.trim() === 'Image');
imgLink ? imgLink.id : 'not found';
// Then: document.getElementById('THAT_ID').click()
```
This opens a PDF in a new tab. Use `tabs_context_mcp` to get the new tab ID, then `get_page_text` to read it.

**What to look for in the complaint:**
- **Assessed value** — listed near the top (e.g. "Assessed Value: $23,000")
- **Property description** — lot size, location, any improvements
- **Original tax lien amount** — the amount owed at time of filing

**Equity threshold — minimum $75k in equity to be worth pursuing:**

Estimated Equity = Assessed Value − Total Tax Debt

The complaint shows the original lien amount, but taxes accrue interest and fees over time. If the case is several years old, estimate the current tax burden:
- Look at how many years of taxes are unpaid
- MA tax liens accrue interest at **16% annually before 2023**, and **8% annually from 2023 onward**
- Example: a $10k lien filed in 2018 → ~4 years at 16% + ~2 years at 8% = roughly $18–20k total by 2026
- If assessed value is $100k and taxes have been unpaid for years, estimate whether total debt has grown past $25k

**Quick filter logic:**
- Assessed value < $75k → **skip immediately** (can't have $75k equity no matter what)
- Assessed value $75k–$150k → **estimate tax debt carefully** — only proceed if you believe equity > $75k
- Assessed value > $150k → **likely worth researching** unless taxes are extremely high

**Only proceed with full case analysis if estimated equity clears $75k.**

**Step 5 — Read the full case page (only if value checks out):**
```javascript
...
| "Notice of payment agreement filed" + no further action | Payments are likely being made — flag WATCH, not HOT/WARM. Monitor for new docket entries (motion, default, new checklist) which signal the plan broke down. |
...
- "Notice of payment agreement filed" in docket with no further court action — payments likely being made, monitor for default
...
### Step 0 — ArcGIS assessed value check (do this FIRST — 10-second filter)

**Before opening any images or tracing heirs**, look up the property address in the Massachusetts ArcGIS parcel layer to get the current assessed value. A low value means the property is upside down — skip immediately.
...
**Query (run from `javascript_tool` in Chrome, or via Python bash curl):**
```javascript
const addr = '123 MAIN ST';  // property address from MassCourts, uppercase
const city = 'SPRINGFIELD';  // town name, uppercase
const stripped = addr.toUpperCase().replace(/\b(ST|AVE|RD|DR|LN|WAY|BLVD|CT|PL|TER|CIR)\b.*/, '').trim();
const where = encodeURIComponent(`SITE_ADDR LIKE '${stripped}%' AND CITY = '${city}'`);
const url = `https://services1.arcgis.com/hGdibHYSPO59RG1h/arcgis/rest/services/Massachusetts_Property_Tax_Parcels/FeatureServer/0/query?where=${where}&outFields=SITE_ADDR,CITY,ZIP,OWNER1,TOTAL_VAL,LAND_VAL,BLDG_VAL,OWN_ADDR,OWN_CITY,OWN_STATE,OWN_ZIP&f=json`;
fetch(url).then(r=>r.json()).then(d=>{ window._av = d.features?.[0]?.attributes; console.log(JSON.stringify(window._av)); });
```

**Key field:** `TOTAL_VAL` = total assessed value (land + building).

**⚠️ Field notes (verified 04/2026):** Use `CITY` not `TOWN`. `OWNER2` no longer exists. Always use `encodeURIComponent()` — never `URLSearchParams` (encodes spaces as `+`, which ArcGIS rejects).

**Decision rule:**
- `TOTAL_VAL` ≥ ~$75,000 → continue to Step 1
- `TOTAL_VAL` < ~$75,000 → **SKIP** — upside down given accumulated taxes + 16%/yr interest + legal fees. Don't open PDFs or trace heirs.

If address doesn't match, fallback: `UPPER(OWNER1) LIKE '%LASTNAME%' AND CITY = 'CITYNAME'`

---

### Step 1 — Open the complaint image: confirm assessed value and get tax taking year
...
- **Assessed value of the property** (confirm against ArcGIS — complaint value is as of filing date)
- **Tax taking year** — the year the town first took the tax lien (start of interest calculation)
...
For every case that makes it past the dead-end filter (not just HOT):
1. **Open the complaint image and title report image** from the docket before forming any assessment — click the Image links via `document.getElementById(id).click()` to trigger the Wicket AJAX calls. The title report (Sheet 2 / Rider) often contains the full ownership chain, probate details, death certificates, will provisions, and heir addresses that completely change the picture.
2. **Run SmartSkip** on the property address (`https://app.smartskip.io/manual-skip`) — confirms who is actually living there, shows address history, and pulls phone numbers for all associated parties.

For each HOT lead, additionally:
3. **Run Phase 3** — complaint image (equity check) → title report (deed + balance) → obit chain → probate search for any Trust defendants
4. **Identify the 2–4 real decision-makers** and find their phone numbers (WhitePages, BeenVerified)
5. **Google Street View** the property — look for occupancy, condition, neglect
6. **Check masslandrecords.com** if the title report flagged anything unusual about the deed chain
...
| lives there / heir lives there | Skip — someone is occupying. **Exception:** if the occupant holds only a life estate (not fee simple), this is NOT a straight skip — flag WATCH. A life estate holder has a right to stay while alive but the remaindermen also have interests and the life tenant may have power to sell. |
| payment agreement / payment plan | Skip in Jordan's notes = deal already in progress. But "Notice of payment agreement filed" appearing in a fresh docket (not in Jordan's notes) = WATCH, not skip. |
